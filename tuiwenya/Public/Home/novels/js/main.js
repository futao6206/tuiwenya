...
$(document).ready(function () {
        ...
        if ($(this).hasClass('add-color')) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.status == 1) {
                        $('#tuiwenya_book').removeClass('add-color');
                    }
                    if (data.status == -1) {
                        alert('您还没有登录');
                    }
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.status == 1) {
                        $('#tuiwenya_book').addClass('add-color');
                        alert('已加入书架^_^');
    ...
    $('#bookMark').click(function () {
        ...
        bookmarkurl = "/tuiwenya_content?url=" + url + "&name=" + content_name + "&chapter_url=" + chapter_url + "&novels_name=" + novels_name;
    ...
    // login
    $("#tuiwenya_login").click(function () {
        var tuiwenya_user = $("#tuiwenya_user").val();
        var tuiwenya_pass = $("#tuiwenya_pass").val();
        if (tuiwenya_user == "" || tuiwenya_pass == "") {
            alert('不能有内容为空');
        } else {
            var login_pd = {'user': tuiwenya_user, 'pwd': tuiwenya_pass};
            $.ajax({
                type: "post",
                contentType: "application/json",
                url: "/operate/login",
